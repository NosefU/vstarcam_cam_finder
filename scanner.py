...
class Camera:
    def __init__(self, response: bytes):
        ...
        self.ip = self._get_bytes(offset, length).decode("ascii").rstrip('\x00')
        ...
        self.netmask = self._get_bytes(offset, length).decode("ascii").rstrip('\x00')

        # gateway
        offset, length = int('24', 16), 15
        self.gateway = self._get_bytes(offset, length).decode("ascii").rstrip('\x00')

        # dns1
        offset, length = int('34', 16), 15
        self.dns1 = self._get_bytes(offset, length).decode("ascii").rstrip('\x00')

        # dns2
        offset, length = int('44', 16), 15
        self.dns2 = self._get_bytes(offset, length).decode("ascii").rstrip('\x00')
        ...
        self.cloud_id = self._get_bytes(offset, length).decode("ascii").rstrip('\x00')

        # name
        offset, length = int('7C', 16), 31
        self.name = self._get_bytes(offset, length).decode("utf-8").rstrip('\x00')

        # firmware version
        offset, length = int('CC', 16), 15
        self.firmware = self._get_bytes(offset, length).decode("ascii").rstrip('\x00')

        # mode
        offset, length = int('DC', 16), 15
        mode = self._get_bytes(offset, length).decode("ascii").rstrip('\x00')
        self.mode = mode[13]
